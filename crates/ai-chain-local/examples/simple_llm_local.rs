...
use ai_chain::options::{ModelRef, Options};
use std::{env::args, error::Error};

use ai_chain::{prompt::Data, traits::Executor};

extern crate ai_chain_local;

/// This example demonstrates how to use the ai-chain-local crate to generate text using a model.
///
/// Usage: cargo run --release --package ai-chain-local --example simple model_type path/to/model
///
/// For example, if the model is a LLaMA-type model located at "/models/llama"
/// cargo run --release --package ai-chain-local --example simple llama /models/llama
///
