...
pub enum Opt {
    ...
    /// This is known as logit bias in OpenAI and is also used in ai-chain-local.
    TokenBias(TokenBias),
...
mod tests {
    ...
    fn test_options_from_env() {
        ...
        env::set_var("ai_chain_MODEL", orig_model);
        env::set_var("ai_chain_N_BATCH", orig_nbatch.to_string());
        env::set_var("ai_chain_API_KEY", orig_api_key);
