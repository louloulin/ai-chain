...
use ai_chain::output::parser::{MarkdownParser, OutputParser};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = res.to_immediate().await?;
    let parser = MarkdownParser::new();
    let data:String = output.into();
    println!("{}",parser.parse(data.as_str()).await?);
    Ok(())
}
