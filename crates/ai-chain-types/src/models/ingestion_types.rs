...
pub enum IngestionMessage {
    ...
    /// A connector uses this message kind to notify AIChain that a initial snapshot of the source tables is started
    SnapshottingStarted,
    /// A connector uses this message kind to notify AIChain that a initial snapshot of the source tables is done,
...
#[derive(Debug, Serialize, Deserialize, Eq, PartialEq, Clone, Hash, JsonSchema)]
pub struct NestedAIChainConfig {
    pub url: String,
    #[serde(default, skip_serializing_if = "equal_default")]
    pub log_options: NestedAIChainLogOptions,
}

#[derive(Debug, Serialize, Deserialize, Eq, PartialEq, Clone, Hash, JsonSchema, Default)]
pub struct NestedAIChainLogOptions {
    #[serde(skip_serializing_if = "Option::is_none")]
