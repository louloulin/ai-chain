use crate::types::{AIChainDuration, AIChainPoint, Field};
...
impl From<Point<OrderedFloat<f64>>> for Field {
    fn from(value: Point<OrderedFloat<f64>>) -> Self {
        Field::Point(AIChainPoint(value))
    }
}

impl From<Point> for Field {
    fn from(value: Point) -> Self {
        let v = AIChainPoint::from((value.x(), value.y()));
        Field::Point(v)
    }
}

impl From<AIChainDuration> for Field {
    fn from(value: AIChainDuration) -> Self {
        let v = AIChainDuration(value.0, value.1);
        Field::Duration(v)
    }
}
