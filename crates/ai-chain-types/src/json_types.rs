...
use crate::types::{AIChainDuration, Field, DATE_FORMAT};
...
fn convert_duration_to_object(duration: AIChainDuration) -> JsonValue {
    let mut object = JsonObject::new();
...
mod tests {
    ...
    use crate::{
        ...
        types::{AIChainPoint, Field, FieldType, TimeUnit},
    };
    ...
    fn test_field_types_json_conversion() {
        let fields = vec![
            ...
            (
                FieldType::Point,
                Field::Point(AIChainPoint::from((3.234, 4.567))),
            ),
            (
                FieldType::Duration,
                Field::Duration(AIChainDuration(
