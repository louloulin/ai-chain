use crate::types::{field_test_cases, AIChainDuration, AIChainPoint, Field, TimeUnit};
...
fn test_as_conversion() {
    ...
    let field = Field::Point(AIChainPoint::from((0.0, 0.0)));
    assert!(field.as_uint().is_none());
    assert!(field.as_int().is_none());
    assert!(field.as_u128().is_none());
    assert!(field.as_i128().is_none());
    ...
    let field = Field::Duration(AIChainDuration(
