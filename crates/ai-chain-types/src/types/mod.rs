...
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
pub struct AIChainDuration(pub std::time::Duration, pub TimeUnit);

impl Ord for AIChainDuration {
    fn cmp(&self, other: &Self) -> Ordering {
        std::time::Duration::cmp(&self.0, &other.0)
    }
}

impl PartialOrd for AIChainDuration {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl FromStr for AIChainDuration {
    type Err = TypeError;
...
pub struct AIChainPoint(#[bincode(with_serde)] pub Point<OrderedFloat<f64>>);

#[cfg(feature = "arbitrary")]
impl<'a> arbitrary::Arbitrary<'a> for AIChainPoint {
...
impl GeodesicDistance<OrderedFloat<f64>> for AIChainPoint {
...
impl PartialOrd for AIChainPoint {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl FromStr for AIChainPoint {
    type Err = TypeError;
...
impl From<(f64, f64)> for AIChainPoint {
    fn from((x, y): (f64, f64)) -> Self {
        Self(point! {x: OrderedFloat(x), y: OrderedFloat(y)})
    }
}

impl Display for AIChainPoint {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
