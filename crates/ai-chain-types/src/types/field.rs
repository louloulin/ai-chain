...
use crate::types::{
    AIChainDuration, AIChainPoint, FieldDefinition, Schema, SourceDefinition, TimeUnit,
};
...
impl bincode::Decode for Field {
    fn decode<D: bincode::de::Decoder>(
        ...
        match first_byte {
            ...
            13 => Ok(Field::Point(AIChainPoint::decode(decoder)?)),
            14 => Ok(Field::Duration(AIChainDuration::decode(decoder)?)),
            15 => Ok(Field::Null),
...
impl<'de> bincode::BorrowDecode<'de> for Field {
    fn borrow_decode<D: bincode::de::BorrowDecoder<'de>>(
        ...
        match first_byte {
            ...
            13 => Ok(Field::Point(AIChainPoint::borrow_decode(decoder)?)),
            14 => Ok(Field::Duration(AIChainDuration::borrow_decode(decoder)?)),
            15 => Ok(Field::Null),
            other => Err(bincode::error::DecodeError::UnexpectedVariant {
                type_name: "Field",
                allowed: &bincode::error::AllowedEnumVariants::Range { min: 0, max: 15 },
...
impl Field {
    ...
    pub fn decode(buf: &[u8]) -> Result<Field, DeserializationError> {
        ...
        match first_byte {
            ...
            13 => Ok(Field::Point(
                AIChainPoint::from_bytes(val).map_err(|_| DeserializationError::BadDataLength)?,
            )),
            14 => Ok(Field::Duration(
                AIChainDuration::from_bytes(val).map_err(|_| DeserializationError::BadDataLength)?,
            )),
            15 => Ok(Field::Null),
    ...
    pub fn as_point(&self) -> Option<AIChainPoint> {
        match self {
    ...
    pub fn as_duration(&self) -> Option<AIChainDuration> {
        match self {
            Field::UInt(d) => Some(AIChainDuration(
                Duration::from_nanos(*d),
                TimeUnit::Nanoseconds,
            )),
            Field::U128(d) => Some(AIChainDuration(
                Duration::from_nanos(*d as u64),
                TimeUnit::Nanoseconds,
            )),
            Field::Int(d) => Some(AIChainDuration(
                Duration::from_nanos(*d as u64),
                TimeUnit::Nanoseconds,
            )),
            Field::I128(d) => Some(AIChainDuration(
                Duration::from_nanos(*d as u64),
                TimeUnit::Nanoseconds,
            )),
    ...
    pub fn to_point(&self) -> Option<AIChainPoint> {
        match self {
    ...
    pub fn to_duration(&self) -> Option<AIChainDuration> {
        match self {
            Field::UInt(d) => Some(AIChainDuration(
                Duration::from_nanos(*d),
                TimeUnit::Nanoseconds,
            ...
            Field::Duration(d) => Some(*d),
            Field::String(d) | Field::Text(d) => AIChainDuration::from_str(d.as_str()).ok(),
            Field::Null => Some(AIChainDuration(
...
/// All field types supported in AIChain.
pub enum FieldType {
...
/// and we need this function in `AIChain-cache`.
