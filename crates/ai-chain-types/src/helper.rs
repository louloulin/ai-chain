...
use crate::types::{AIChainDuration, AIChainPoint, TimeUnit, DATE_FORMAT};
                ...
                Some(Value::String(tu_val)) => {
                    ...
                    return match time_unit {
                        TimeUnit::Seconds => {
                            ...
                            Ok(Field::Duration(AIChainDuration(
                        ...
                        TimeUnit::Nanoseconds => {
                            Ok(Field::Duration(AIChainDuration::from_str(v_val).unwrap()))
                        }
                    };
                }
                _ => Err(DeserializationError::Custom(
        ...
        match typ {
            ...
            FieldType::Point => {
                if nullable && (value.is_empty() || value == "null") {
                    ...
                    value.parse::<AIChainPoint>().map(Field::Point)
                }
            }
            FieldType::Duration => {
                if nullable && (value.is_empty() || value == "null") {
                    ...
                    value.parse::<AIChainDuration>().map(Field::Duration)
                }
            }
        }
    }
    ...
    fn test_field_from_str() {
        let ok_cases = [
            ...
            (
                ...
                FieldType::Point,
                false,
                Field::Point(AIChainPoint(Point::new(OrderedFloat(1.0), OrderedFloat(1.0)))),
            ),
            (
                "{\"abc\":\"foo\"}",
                FieldType::Json,
